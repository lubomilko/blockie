...
Automatic :ref:`variables and blocks <tgt_tags>` are filled automatically without any values
specified in the input data. The automatic elements typically help with the formatting of the
generated content.
...
left-alignment of a text within the template having :ref:`variables <tgt_variable_oper>` filled
with values of different character lengths.
...
A block content can be :ref:`set <tgt_set_blk_cont>` into a different location than its original
location, or into multiple different locations within the template using a special tag having a
``<@BLOCK_NAME>`` format. This so-called *block variable* acts as a target for a block content,
while the original block is automatically :ref:`cleared <tgt_clear_blk>`.
